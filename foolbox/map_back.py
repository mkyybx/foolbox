...
def run_cpp_feature_extractor(domain_url, working_dir, parse_modified=True):
    def execute_shell_command(cmd):
        os.system(cmd)

    cmd_lst = []
    cmd_lst.append("cd %s" % working_dir)
    if not parse_modified:
...
def compute_x_after_mapping_back(domain_url, url_id, modified_json, original_json_fname, working_dir, first_time=False):
    # input(working_dir)
    if first_time:
